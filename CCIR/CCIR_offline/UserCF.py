...
def UserSimilarity(User_behavior):
    ...
    np.savetxt('E:\\CCIR\\ml-latest-small\\UserCF_Weight.txt', SimilarityWeight, fmt='%f', delimiter=' ',
               newline='\r\n')
...
def PrintRecommendlist():
    filePath = 'E:\\CCIR\\ml-latest-small\\ratings.csv'
    OutFilePath = "E:\\CCIR\\ml-latest-small\\User_Recommend.txt"
    OutFilePath_all = "E:\\CCIR\\ml-latest-small\\User_Recommend_all.txt"
    Out_File_object = open(OutFilePath, 'w', encoding="UTF-8")
    Out_File_object_all = open(OutFilePath_all, 'w', encoding="UTF-8")
    User_behavior = loadData(filePath)
    User_Similarity_Weight = UserSimilarity(User_behavior)
    # 用户ID 1-671
    # 将获得的推荐字典按照Value排序
    for user_ID in User_behavior.keys():
        recommendlist = Recommend(user_ID, 15, User_Similarity_Weight, User_behavior)
        ...
        for index in range(n):
            if count < n-1:
                RecItem = RecItem + dic[index][0] + ','
                allLine = allLine + "(" + dic[index][0] + "," + str(dic[index][1]) + "),"
                # allLine = allLine + k
                count += 1
            elif count == n-1:
                RecItem = RecItem + dic[index][0]
        ...
        Out_File_object.write(RecItem+"\n")
        Out_File_object_all.write(allLine+"\n")
        print(user_ID, '>>>>', RecItem)

    Out_File_object.close()
    Out_File_object_all.close()
