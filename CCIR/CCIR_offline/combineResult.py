...
def get_ItemCF(small_test_file):
    ItemCF_csv = open("F:\\CCIR_eval\\test_eval_result.csv", 'r', encoding='UTF-8')
    test_ID = load_small_testID(small_test_file)
    ItemCF_dic = dict()
    index = 0
    for item in ItemCF_csv:
        tt = item.strip('\n').split(',')
        simply_item_list = list()
        for i in range(0, len(tt) - 1):
            if tt[i] == '-1':
                break
            else:
                simply_item_list.append(tt[i])
        ItemCF_dic[test_ID[index]] = simply_item_list
        index += 1
    ItemCF_csv.close()
...
def get_UserCF(small_test_file):
...
def get_Popular(small_test_file):
    Popular_csv = open("F:\\CCIR_eval\\popular_result.csv", 'r', encoding='UTF-8')
    test_ID = load_small_testID(small_test_file)
    Popular_dic = dict()
    index = 0
    for user in Popular_csv:
        tt = user.strip('\n').split(',')
        simply_user_list = []
        for i in range(0, len(tt) - 1):
            if tt[i] == '-1':
                ...
                simply_user_list.append(tt[i])
        Popular_dic[test_ID[index]] = simply_user_list
        index += 1
    Popular_csv.close()
    print(f"Popular_dic数量:{len(Popular_dic)}")
    return Popular_dic


if __name__ == '__main__':
    # baseOnQuestion_csv = open("E:\\CCIR\\Combine_final\\CCIR_baseOnQuestion_result.csv", 'r', encoding='UTF-8')
    small_test_file = "F:\\CCIR_eval\\CCIR_test_eval.txt"
    csv_out_file = "F:\\CCIR_eval\\test_combin_result.csv"
    test_ID = load_small_testID(small_test_file)
    ItemCF_dic = get_ItemCF(small_test_file)
    Popular_dic = get_Popular(small_test_file)
    Combine_dic = dict()
    for item in ItemCF_dic.keys():
        ItemCF = ItemCF_dic[item]
        # print(ItemCF)
        temp_Item = ItemCF[:50]
        temp_poluar = Popular_dic[item][:50]
        for a in temp_Item:
            if a in temp_poluar:
                temp_poluar.remove(a)
        temp_Item.extend(temp_poluar)
        Combine_dic[item] = temp_Item
