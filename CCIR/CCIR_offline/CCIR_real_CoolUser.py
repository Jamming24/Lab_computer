...
def get_question_inverse_answer(candidate_answer_file, question_inverse_answer_file):
    file_object = open(candidate_answer_file, 'r', encoding='UTF-8')
    outfile_object = open(question_inverse_answer_file, 'w', encoding='UTF-8')
    question_inverse_answer_dic = dict()
    for line in file_object:
        tt = line.split('\t')
        answerID = tt[0]
        questionID = tt[1]
        if questionID in question_inverse_answer_dic.keys():
            temp = question_inverse_answer_dic[questionID]
            temp.append(answerID)
        else:
            answer_list = [answerID]
            question_inverse_answer_dic[questionID] = answer_list
    file_object.close()
    count = 0
    for key in question_inverse_answer_dic.keys():
        outfile_object.write(key+"\t")
        count += len(question_inverse_answer_dic[key])
        for item in question_inverse_answer_dic[key]:
            outfile_object.write(item+",")
        outfile_object.write('\n')
    print(f"问题数量{len(question_inverse_answer_dic)}")
    print("打印完成")
    print(f"答案数量{count}")
...
if __name__ == '__main__':
    ...
    get_question_inverse_answer(candidate_answer_file, question_inverse_answer_file)