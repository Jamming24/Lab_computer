...
def get_User_read_topic(User_Behavior_Dict, answer_id_dict_file, answer_map_topic_file):
    answer_id_dict = load_answer_dict(answer_id_dict_file)
    answer_map_topic_file_object = open(answer_map_topic_file, 'r', encoding='UTF-8')
    user_read_topics_dic = dict()
    answer_map_topic_dic = dict()
    for line in answer_map_topic_file_object:
        tt = line.split('\t')
        if len(tt[1]) > 30:
            answer_map_topic_dic[tt[0]] = tt[1].strip('\n').split(',')
    answer_map_topic_file_object.close()
    print(f"answer_map_topic_dic数量大小:{len(answer_map_topic_dic)}")
    for user in User_Behavior_Dict.keys():
        Simply_user_read_topic_dic = dict()
        for item in User_Behavior_Dict[user]:
            ...
            if shortId in answer_id_dict.keys():
                ...
                if LongID in answer_map_topic_dic.keys():
                    topics = answer_map_topic_dic[LongID]

                    for topic in topics:
                        if topic in Simply_user_read_topic_dic.keys():
                            topic_score = Simply_user_read_topic_dic[topic]
                            topic_score += shortId_Score
                            Simply_user_read_topic_dic[topic] = topic_score
                        else:
                            Simply_user_read_topic_dic[topic] = shortId_Score
        user_read_topics_dic[user] = Simply_user_read_topic_dic
    return user_read_topics_dic


def get_baseOn_topic_RecList(User_Behavior_Dict, user_read_topics_dic, topic_inverse_answer_dic, candidate_answer_quality, recfile):
    ...
    for user in User_Behavior_Dict.keys():
        ...
        # 计算用户推荐列表
        if user in user_read_topics_dic.keys():
            topics = user_read_topics_dic[user]
            for topicID in topics.keys():
                topic_score = topics[topicID]
                if topicID in topic_inverse_answer_dic.keys():
                    topic_map_answers = topic_inverse_answer_dic[topicID]
...
def multiprocessing_manager(n, User_Behavior_Dict, user_read_topics_dic, topic_inverse_answer_dic, candidate_answer_quality, OutfileFloder):
    ...
    for userID in User_Behavior_Dict.keys():
        if len(temp_User_Behavior_Dict) == 10000:
            ...
            result = pool.apply_async(get_baseOn_topic_RecList,
    ...
    result = pool.apply_async(get_baseOn_topic_RecList, args=(last_part_User_Behavior_Dict, user_read_topics_dic, topic_inverse_answer_dic, candidate_answer_quality, recfile))
    result_list.append(result)
    temp_User_Behavior_Dict.clear()
    pool.close()
    pool.join()
...
if __name__ == '__main__':
    ...
    answer_map_topic_file = "E:\\CCIR\\answer_map_topic.txt"
    ...
    csv_out_file = "E:\\CCIR\\CCIR_baseOnTopic_result.csv"
    recfile = "E:\\CCIR\\\CCIR_baseOnTopic_RecScore"
    User_Behavior_Dict, test_ID = load_small_test(small_test_file)
    user_read_topics_dic = get_User_read_topic(User_Behavior_Dict, answer_id_dict_file, answer_map_topic_file)
